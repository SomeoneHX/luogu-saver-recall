async function pagePaste(params) {
  const id = params.id;
  const el = document.getElementById('page-content');
  el.innerHTML = `
    <input type="hidden" id="url" value="paste/` + id + `">
    <input type="hidden" id="markdown-content" value="">
    <div class="ui grid">
      <div class="sixteen wide column">
        <div class="card shadow outline">
          <h1 class="ui header">剪贴板 - ` + id + `</h1>
          <span class="meta gray" id="paste-meta"><i class="ui icon calendar colored"></i> </span>
          <div class="ui grid">
            <div class="sixteen wide column">
              <div class="card shadow outline padding-15" style="height: 90%;">
                ...
                <div class="meta user" style="margin-top: 5px;">
                  <img class="ui mini circular image" id="paste-avatar" src="https://cdn.luogu.com.cn/upload/usericon/3.png" alt="user_icon">
                  <a id="paste-author" href="javascript:void(0)" style="margin-left: 5px; font-weight: 550;"></a>
                </div>
              </div>
            </div>
            <div class="sixteen wide column">
              <a href="javascript:void(0)" id="paste-original-link" class="ui button" style="display:none;">
                <i class="ui icon share square"></i> 查看原剪贴板
              </a>
              <a href="javascript:void(0)" onclick="copyMarkdown()" class="ui button" id="paste-copy-btn" style="display:none;">
  ...
  try {
    ...
    if (data.author) {
      ...
      const authorEl = document.getElementById('paste-author');
  ...
  document.getElementById('save-btn')?.addEventListener('click', async function () {
    try {
      ...
      const result = await api.post('/workflow/create/template/paste-save-pipeline', { targetId: id });
      sessionStorage.setItem('wt:' + result.workflowId, JSON.stringify({ targetId: id, type: 'paste' }));
