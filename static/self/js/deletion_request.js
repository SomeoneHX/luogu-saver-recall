async function requestDeletion(targetType, targetId) {
  const result = await Swal.fire({
    ...
    didOpen: () => {
      ...
      if (warningMsg) {
        ...
        let nxtColor = 0;
        setInterval(() => {
          colorClass.forEach(color => { warningMsg.classList.remove(color); });
  ...
  if (result.isConfirmed) {
    ...
    try {
      ...
      await api.post('/deletion-request', { targetType, targetId, reason });
      Swal.fire({
        title: '提交成功',
        text: '删除申请已提交，请等待管理员审核',
        icon: 'success',
      ...
      Swal.fire({
        title: '提交失败',
        text: err.message || '网络错误或服务器异常，请稍后再试',
        icon: 'error',
        confirmButtonText: '确定'
      });
    }
